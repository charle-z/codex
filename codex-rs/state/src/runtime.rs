    ...
    async fn init_inner(
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                close_sqlite_pools_with_optional_logs(&[pool.as_ref()], logs_pool.as_deref())
                    .await;
                return Err(err);
            }
        };
...
mod tests {
        ...
        assert!(
            !telemetry.counters().iter().any(|event| {
                event.name == DB_FALLBACK_METRIC
                    && event.tags.get("caller").map(String::as_str)
                        == Some("state_runtime_init")
            }),
    ...
    async fn locked_logs_db_does_not_gate_state_but_blocks_strict_thread_delete() {
        ...
        let thread_id = ThreadId::from_string("00000000-0000-0000-0000-000000000355")
